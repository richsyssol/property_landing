...
if (isset($_GET['id'])) {
    $id = intval($_GET['id']);
    $query = "SELECT * FROM `whyinvest` WHERE `id` = $id";
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        echo "<script>alert('whyinvest not found'); window.location.href='whyinvest.php';</script>";
        exit();
    }
} else {
    echo "<script>alert('Invalid request'); window.location.href='whyinvest.php';</script>";
    exit();
}
...
if (isset($_POST['update'])) {
    ...
    if (!empty($image)) {
        ...
        if (!in_array(strtolower($file_extension), $allowed_extensions)) {
            ...
            $update_query = "UPDATE `whyinvest` SET `image` = '$upload_file', `title` = '$title', `status` = '$status', property_type = '$property_type' WHERE `id` = $id";
        }
    } else {
        // Update database without changing the image
        $update_query = "UPDATE `whyinvest` SET `title` = '$title', `status` = '$status', property_type = '$property_type' WHERE `id` = $id";
    }

    $update_result = mysqli_query($conn, $update_query);
    
    if ($update_result) {
        echo '<script>window.location.href = "whyinvest.php";</script>';
        exit();
    } else {
    ...
    <div class="container">
        <form action="" method="post" enctype="multipart/form-data">
            <div class="mb-3">
                ...
                <img src="<?php echo $existing_image; ?>" alt="whyinvest Image" width="150">
            </div>
